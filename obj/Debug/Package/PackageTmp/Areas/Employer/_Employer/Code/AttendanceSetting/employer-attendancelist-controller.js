...
app.controller("employer-attendancelist-controller", ['$scope', 'httpService', 'httpCommonService',
    function ($scope, httpService, httpCommonService) {
        ...
        if ($scope._izememployerbranchId != 0)
            $scope.isBrachLogin = true;
        else
            $scope.isBrachLogin = false;
        ...
        $scope.onLoad_Access = (localData) => {
            try {
                ...
                if (_subDiv.length > 0) {
                    let _layout = _subDiv.filter(x => x.menuName == 'Attendance entries' && x.pageName == 'Attendance');
                    if (_layout.length > 0) {
        ...
        $scope.onLoad_Clear = () => {
            ...
            if ($scope._izememployerbranchId != 0)
                $scope.SearchEmployerbranchId = $scope._izememployerbranchId;
            else
                ...
                httpService.httpFetchData(req)
                    .then((res) => {
                        if (res.status == 200) {
                            ...
                            $scope.employeeattendanceStatus = res.data.employeeattendanceStatus;
        ...
        $scope.onLoad_MasterTransfer = () => {
            try {
                ...
                let req = {
                    ...
                    data: {
                        "employerId": $scope._izemEmployerId,
                        "branchId": $scope._izememployerbranchId,
                    ...
                    .then((res) => {
                        if (res.status == 200) {
                            ...
                            if (obj.length > 0) {
                                ...
                                strstring += "<td x:autofilter='all'><b> Location/Machine </b></td>";
                                ...
                                for (let i = 0; i < obj.length; i++) {
                                    ...
                                    let _employeeattendanceStatus = obj[i].employeeattendanceStatus == null ? '-' : obj[i].employeeattendanceStatus;
                                    ...
                                    strstring += "<td x:autofilter='all'> " + _employeeattendanceStatus + " </td>";
