...
app.controller("employer-leaveapplication-controller", ['$scope', 'httpService', 'httpCommonService', '$http',
    function ($scope, httpService, httpCommonService, $http) {
        ...
        $scope._izemLoginName = localStorage.getItem("_izemFullName");
        $scope._izemLoginEmail = localStorage.getItem("_izemLoginEmail");
        $scope._izemEmployerEmail = localStorage.getItem("_izemEmployerEmail");
        $scope._izemEmployerEmail = localStorage.getItem("_izemEmployerManagerEmail");
        ...
        $scope.onLoad_Access = (localData) => {
            try {
                ...
                if (_subDiv.length > 0) {
                    let _layout = _subDiv.filter(x => x.menuName == 'Leave entries' && x.pageName == 'Leave Application');
                    if (_layout.length > 0) {
        ...
        $scope.onLoad_Clear = () => {
            ...
            $scope.employeeleaveapplicationLeaveStatus = "Approved";
            $scope.employeeleaveapplicationIsEmergency = "";
        ...
        $scope.onLoad_MasterService = () => {
            ...
            let searchLeaveType = $scope.searchLeaveType;
        ...
        /* Email formatting */
        //. Rejection
        $scope.rejectionEmail = (_selectedEmployeeEmail, _izemLoginName, _selectedEmployeeName, _employeeleaveapplicationLeaveStatus,
            _employeeleaveapplicationLeaveFrom, _employeeleaveapplicationLeaveTo, _selectedLeaveType,
            _lblEntitlementLeave, _employeeleaveapplicationNoOfDays, _reason) => {
            try {

                $http({
                    method: "POST",
                    url: "/employer/LeaveSetting/send_EmailForRejectLeaveApplication",
                    responseType: "blob",
                    data: {
        ...
        //. Approval
        $scope.approveEmail = (_selectedEmployeeEmail, _izemLoginName, _selectedEmployeeName, _employeeleaveapplicationLeaveStatus,
            _employeeleaveapplicationLeaveFrom, _employeeleaveapplicationLeaveTo, _selectedLeaveType,
            _lblEntitlementLeave, _employeeleaveapplicationNoOfDays, _reason, _data) => {
            try {

                $http({
                    method: "POST",
                    url: "/employer/LeaveSetting/send_EmailForApproveLeaveApplication",
                    responseType: "blob",
                    data: {
                        "memberEmail": _selectedEmployeeEmail,
                        "loginName": _izemLoginName,
        ...
        $scope.onClick_Submit = () => {
            try {
                ...
                if ($scope.employeeleaveapplicationIsEmergency == "") {
                    alert("Please provide emergency status!");
                    return;
                }
                ...
                if (employeeleaveapplicationIsPartialday == true) {
                    if (employeeleaveapplicationIsStartPartialday == false && employeeleaveapplicationIsEndPartialday == false) {
                        alert("Please specify the partial day!");
                        return;
                    }
                }
                ...
                httpService.httpOperationData(req)
                    .then((res) => {
                        if (res.status == 200) {
                            ...
                            if (employeeleaveapplicationLeaveStatus == "Rejected") {
                                $scope.rejectionEmail($scope._selectedEmployeeEmail, $scope._izemLoginName, $scope._selectedEmployeeName,
                                    employeeleaveapplicationLeaveStatus, employeeleaveapplicationLeaveFrom, employeeleaveapplicationLeaveTo,
                                    $scope._selectedLeaveType, $scope.lblEntitlementLeave, employeeleaveapplicationNoOfDays, employeeleaveapplicationReasonReject);
                            }
                            if (employeeleaveapplicationLeaveStatus == "Approved" || employeeleaveapplicationLeaveStatus == "Pending") {
                                    ...
                                    httpService.httpFetchData(req)
                                        .then((res) => {
                                            if (res.status == 200) {
                                                ...
                                                if (employeeleaveapplicationLeaveStatus == "Approved") {
                                                    $scope.approveEmail($scope._selectedEmployeeEmail, $scope._izemLoginName, $scope._selectedEmployeeName,
                                                        employeeleaveapplicationLeaveStatus, employeeleaveapplicationLeaveFrom, employeeleaveapplicationLeaveTo,
                                                        $scope._selectedLeaveType, $scope.lblEntitlementLeave, employeeleaveapplicationNoOfDays, employeeleaveapplicationReason, data);
                                                }
                                                else {
            ...
            httpService.httpOperationData(req)
                .then((res) => {
                    if (res.status == 200) {
                        ...
                        let selectedEmployer = $scope.employeeId;

                        $scope.onLoad_Clear();
                        $scope.onClick_AddRecord();
                        $scope.employeeId = selectedEmployer;
                    }
                }, (err) => {
                    console.log(err);
                });
                ...
                httpService.httpFetchData(req)
                    .then((res) => {
                        if (res.status == 200) {
                            ...
                            $scope.employeeleaveapplicationIsPartialday = res.data.employeeleaveapplicationIsPartialday.data[0] == 1 ? true : false;
                            ...
                            $scope.employeeleaveapplicationIsStartPartialday = res.data.employeeleaveapplicationIsStartPartialday.data[0] == 1 ? true : false;
                            $scope.employeeleaveapplicationIsEndPartialday = res.data.employeeleaveapplicationIsEndPartialday.data[0] == 1 ? true : false;
        ...
        $scope.onLoad_Employee = () => {
            try {
                ...
                httpCommonService.fill_employee()
                    .then((res) => {
                        if (res.status == 200) {
                            let _filteredEmployeeData = res.data.filter(x => x.employeeLeaving == null);
                            $scope.fill_employeeId = _filteredEmployeeData;
                            $scope.fill_searchMasterEmployeeId = res.data;
                        }
                    }, (err) => {
                        console.log(err);
                    })
            ...
            try {
                ...
                if (employeeData.length > 0) {
                    ...
                    if (employeeData[0].employeeIsFixJoining.data[0] == "0") {
                        ...
                        if ($scope._selectedemployeeManagerId != "") {
                            $scope._selectedEmployeeManagerEmail = employeeData[0].employeeManagerId;
                        }
                        else {
                            $scope._selectedEmployeeManagerEmail = localStorage.getItem("_izemEmployerEmail");;
                        }
        ...
        $scope.onClick_UpdateDateChange1 = () => {
            try {
                let applyDate = $scope.employeeleaveapplicationLeaveFrom;
                if (applyDate != undefined) {
                    $scope.employeeleaveapplicationLeaveTo = new Date(applyDate);
                    $scope.onChange_LeaveCount();
                }
        ...
        $scope.onChange_LeaveType = () => {
            ...
            try {
                ...
                if (entitlementData.length > 0) {
                    ...
                    //. Email
                    $scope._selectedLeaveType = entitlementData[0].employerleavetypeLeaveCode;
